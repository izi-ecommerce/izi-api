...
test: 
	python sandbox/manage.py test iziapi --settings=sandbox.settings.nomigrations

coverage:
	coverage run sandbox/manage.py test iziapi --settings=sandbox.settings.nomigrations
	coverage report -m
	coverage xml -i

docs: install
	pip install "Django>=1.11.0,<=2.2"
